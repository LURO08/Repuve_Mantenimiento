...
");
$pdo->exec("
    CREATE INDEX IF NOT EXISTS idx_infra_revision_evidencias_revision_id
    ON infraestructura_revision_evidencias (revision_id)
");
...
case 'get_materiales':
    ...
    $stmt = $pdo->prepare("
        SELECT am.id AS relacion_id, am.material_id AS id, m.medida AS medida, m.nombre AS material, am.cantidad, am.serie, am.ip, am.mac, m.foto
    ...
    $historialStmt = $pdo->prepare("
        SELECT rm.arco_material_id AS relacion_id, rm.material_id AS id, rm.cantidad, rm.serie, rm.ip, rm.mac, rm.accion,
...
case 'get_infra_materiales':
    ...
    $stmt = $pdo->prepare("
        SELECT im.id AS relacion_id, im.material_id AS id, m.medida AS medida, m.nombre AS material, im.cantidad, im.serie, im.ip, im.mac, m.foto
...
case 'add':
    try {
        ...
        $stmt = $pdo->prepare("SELECT ubicacion_id, COALESCE(estado, 'Activo') AS estado FROM arcos WHERE id = ?");
        $stmt->execute([$arco_id]);
        ...
        if (($arco['estado'] ?? 'Activo') === 'Baja') {
            header("Location: ../views/revisiones.php?error=" . urlencode("No se puede registrar mantenimiento a un arco dado de baja."));
            exit;
        }
        ...
        // Insertar materiales
        $stmt = $pdo->prepare("
            INSERT INTO revision_material (revision_id, arco_material_id, material_id, cantidad, serie, ip, mac, accion)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ");
        ...
        $stmtNuevoMaterialArco = $pdo->prepare("
            INSERT INTO arco_material (arco_id, material_id, cantidad, serie, ip, mac)
            VALUES (?, ?, ?, ?, ?, ?)
            RETURNING id
        ");
